...
void HE_Face::Transform(Matrix4& tranform)
{
	...
	t->vertex00 = (tranform * v0).xyz();
	t->vertex01 = (tranform * v1).xyz();
	t->vertex10 = (tranform * v2).xyz();
...
HE_Face* HE_Face::GetFace(IndexedTriangleSet* triangleSet, int faceIndex)
{
	...
	face->Attributes->PolygonType = PolygonType::PT_Triangles;
...
std::vector<HE_Face*>* HE_Face::GetNearestFaces()
{
	...
	do
	{
		if (edgeInner->Other != nullptr)
		{
			if (std::find(nearestFaces->begin(), nearestFaces->end(), edgeInner->Other->Face) == nearestFaces->end())
			{
